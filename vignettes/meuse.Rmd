...
The following code showcases a very simple example how to use an `sf` object as a `DataBackend` and a direct input into `predict_newdata()`.
...
pred = learner$predict_newdata(newdata, filename = file)
...
# read geopackage which was created during the `predict_newdata()` call
