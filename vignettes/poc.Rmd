...
LearnerClassifRangerParallel

learner_rf = LearnerClassifRangerParallel$new()
...
# TBD: can we write learner_svm$predict(newdata = spatial_data_predict, chunksize = 200, filename = "pred.grd") somehow?
prediction_raster = PredictionRaster$new(data_predict, chunksize = 200)
```
...
RF in non-native parallel mode is very slow.

```{r}
# ras_rf = prediction_raster$predict(learner_rf, filename = "pred.grd")
# ras_rf
```
